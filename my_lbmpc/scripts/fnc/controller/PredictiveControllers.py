...
class LMPC(MPC):
    ...
    #     indexSSandQfun = np.array(indexSSandQfun, dtype=int)
    #     indexSSandQfun = np.clip(indexSSandQfun, 0, x.shape[0] - 1)
    ...
    def selectPoints(self, it, zt, numPoints):
        """
        Robust selection of nearest safe-set points around current state zt.
        Works if self.SS is:
          - list of numpy arrays (each x trajectory)
          - list of dicts {'x':..., 'u':..., 'Qfun':...}
          - numpy ndarray (object dtype)
        Returns: SS_Points (n x numPoints), uSS_Points (d x numPoints), Qfun_Selected (numPoints,)
        """
        import numpy as _np

        # --- debug print once to know the SS structure ---
        if not hasattr(self, "_selectPoints_debug_shown"):
            print(f"[selectPoints] debug: type(self.SS)={type(self.SS)}, len={len(self.SS) if hasattr(self.SS,'__len__') else 'n/a'}")
            if len(self.SS) > 0:
                print(f"[selectPoints] debug: type(self.SS[0])={type(self.SS[0])}")
            self._selectPoints_debug_shown = True

        # ensure integer index for the iteration we want (use it if provided, else last iter)
        try:
            it_idx = int(round(it))
        except Exception:
            it_idx = max(0, int(round(self.it - 1)))

        # Clip iteration index into valid range
        if hasattr(self, "it"):
            it_idx = max(0, min(it_idx, max(0, self.it - 1)))

        # Retrieve x, u, Qfun safely from possible representations
        entry = self.SS[it_idx]

        # Case A: entry is a dict-like (preferred)
        if isinstance(entry, dict):
            x = entry.get("x", entry.get("SS", None))
            u = entry.get("u", entry.get("uSS", None))
            Qfun = entry.get("Qfun", entry.get("Qfun", None))
        else:
            # Case B: entry is likely a numpy array for x; find u and Qfun from parallel containers
            x = _np.asarray(entry)
            u = _np.asarray(self.uSS[it_idx])
            Qfun = _np.asarray(self.Qfun[it_idx])

        # Force numpy arrays
        x = _np.asarray(x)
        u = _np.asarray(u)
        Qfun = _np.asarray(Qfun)

        # s coordinate assumed at index 4
        s_vec = x[:, 4]
        s0 = float(zt[4])
        jj = int(_np.argmin(_np.abs(s_vec - s0)))

        half_window = int(max(1, _np.floor(numPoints / 2)))
        start_idx = jj - half_window
        end_idx = jj + half_window + 1
        idx_array = _np.clip(_np.arange(start_idx, end_idx), 0, x.shape[0] - 1).astype(int)

        SS_Points = x[idx_array, :].T
        uSS_Points = u[idx_array, :].T if u.ndim > 1 else u[idx_array][:, _np.newaxis].T
        Qfun_Selected = Qfun[idx_array]

        return SS_Points, uSS_Points, Qfun_Selected